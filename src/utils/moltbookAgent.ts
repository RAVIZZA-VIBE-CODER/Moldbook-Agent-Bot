...
export type MoltbookConnectionMode = "mock" | "live" | "phone_native";
...
type MoltbookNativePublishResponse = {
  ok: boolean;
  published?: boolean;
  queued?: boolean;
  url?: string;
...
export const MOLTBOOK_TOKEN_FINGERPRINT = "";
export const NEWS_AGENT_DB_KEY = "pocketflow.news.public.agentDb.v1";
export const MOLTBOOK_BUILD_DIARY_KEY = "pocketflow.moldbook.public.buildDiary.v1";
export const MOLTBOOK_BUILD_DIARY_JOB_ID = "moldbook-build-diary-demo";
...
export const MOLTBOOK_DEFAULT_HEALTH: MoltbookConnectionHealth = {
  status: "connected",
  message: "Demo Moldbook runner is armed locally. Wire your own private adapter before posting to a real account.",
  checkedAt: "Saved route",
  endpoint: MOLTBOOK_PHONE_NATIVE_ENDPOINT,
  serverMode: "demo-native",
  modelProvider: "local",
...
const MOLTBOOK_PLATFORM_MAX_PLANNED_RATIO = 0.25;

const isPocketFlowCentricDraft = (draft: Pick<MoltbookPostDraft, "title" | "body" | "pillar" | "source">) =>
  draft.source === "build_diary"
  || /\b(platform|build diary|phone shell|system map|agent map|local ai)\b/i.test(`${draft.title} ${draft.body} ${draft.pillar}`);
...
export const MOLTBOOK_DEFAULT_STATE: MoltbookMobileState = {
  mode: "assisted",
  connectionMode: "phone_native",
  username: MOLTBOOK_ACCOUNT_HANDLE,
  userId: MOLTBOOK_ACCOUNT_USER_ID,
  apiBaseUrl: MOLTBOOK_PHONE_NATIVE_ENDPOINT,
  ...
  commentGuidelines: [
    "Answer comments briefly, warmly, and technically when the comment asks about local AI, builders, dashboards, or workflow design.",
  ...
  interaction: {
    ...
    lastSummary: "Demo phone agent is armed. Moldbook patrol checks throughout the day for posts, replies, useful comments and AI-news drafts.",
  },
  lastLearnedTopic: "Phone migration: keep Moldbook simple, AI-focused, low-load, and autonomous until paused.",
  commandLog: [],
};
...
const stripPrivateBuildDiaryText = (value = "") =>
  value
    ...
    .replace(/moltbook_sk_[A-Za-z0-9_-]+/g, "saved credential")
...
const hydrateMoltbookIdentity = (state: MoltbookMobileState): MoltbookMobileState => {
  ...
  const requestedPhoneNative =
    state.connectionMode === "phone_native"
    || !savedBridge
    || savedBridge === MOLTBOOK_PHONE_NATIVE_ENDPOINT;
  const staleBridge =
    !savedBridge
    || savedBridge.includes("example.invalid")
    || savedBridge.includes("sandbox")
    || savedBridge.includes("localhost")
    || savedBridge.includes("127.0.0.1");
  const normalizedUsername =
    !savedUsername || savedUsername === "@demo_moldbook_bot" || savedUsername === "demo_moldbook_bot"
      ? MOLTBOOK_ACCOUNT_HANDLE
      : savedUsername.replace(/^@+/, "");
  const liveBridge = requestedPhoneNative || staleBridge ? MOLTBOOK_PHONE_NATIVE_ENDPOINT : savedBridge;
  ...
  const staleHealth =
    ...
    || savedHealth.serverMode === "live"
    || savedHealth.endpoint.includes("example.invalid")
  ...
  const connectionHealth: MoltbookConnectionHealth = staleHealth
    ? { ...MOLTBOOK_DEFAULT_HEALTH, endpoint: liveBridge }
    : {
        ...savedHealth,
        endpoint: requestedPhoneNative ? MOLTBOOK_PHONE_NATIVE_ENDPOINT : healthEndpoint,
        serverMode: requestedPhoneNative || savedHealth.serverMode === "live" || savedHealth.serverMode === "mock" ? "phone-native" : savedHealth.serverMode,
        modelProvider: "local",
  ...
  return {
    ...
    connectionMode: requestedPhoneNative || staleBridge || state.connectionMode === "mock" ? "phone_native" : state.connectionMode,
    ...
    visitorPageUrl:
      !visitorPageUrl || visitorPageUrl === "https://moltbook.com" || visitorPageUrl.includes("example.invalid")
...
const pickDiverseReadyDraftIds = (backlog: MoltbookPostDraft[], need: number) => {
  ...
  const pillarCounts = new Map<string, number>();
    const pocketLimit = Math.max(1, Math.floor(Math.max(need, 1) * MOLTBOOK_PLATFORM_MAX_PLANNED_RATIO));
    ...
    : (
        window as Window & {
          MoldbookDemoBridge?: {
            ...
            moltbookPublish?: (payloadJson: string) => Promise<MoltbookNativePublishResponse>;
            moltbookGetStatus?: () => Promise<Record<string, unknown>>;
          };
        }
      ).MoldbookDemoBridge;

const isMoltbookPhoneNativeMode = (state: Pick<MoltbookMobileState, "connectionMode" | "apiBaseUrl">) =>
  state.connectionMode === "phone_native" || state.apiBaseUrl === MOLTBOOK_PHONE_NATIVE_ENDPOINT || !state.apiBaseUrl?.trim();

const buildMoltbookPhoneNativeHealth = (message?: string): MoltbookConnectionHealth => ({
  ...MOLTBOOK_DEFAULT_HEALTH,
  status: "connected",
  message: message || "Moltbook phone-native runner is armed. Remote relay is not the owner of this automation.",
  checkedAt: new Date().toLocaleString(),
  endpoint: MOLTBOOK_PHONE_NATIVE_ENDPOINT,
  serverMode: "phone-native",
  modelProvider: "local",
  modelName: "Baloss LLM",
  modelReady: true,
});

const callMoltbookPhoneNativePublisher = async (
  draft: MoltbookPostDraft,
  reason?: string,
): Promise<MoltbookNativePublishResponse> => {
  const bridge = getMoltbookNativeBridge();
  if (!bridge?.moltbookPublish) {
    return {
      ok: false,
      queued: true,
      published: false,
      message: "Phone-native Moltbook executor is not exposed in this build yet. Draft remains queued locally; external relay was not used.",
    };
  }

  return bridge.moltbookPublish(JSON.stringify({
    id: draft.id,
    title: draft.title,
    body: draft.body,
    content: `${draft.title}\n\n${draft.body}`,
    pillar: draft.pillar,
    source: reason || "pocketflow-phone-native",
    scheduledFor: draft.scheduledFor || new Date().toISOString(),
  }));
};
...
const buildHealthFromMoltbookStatus = (
  ...
  return {
    ...
    message:
      status === "connected"
        ? serverManaged
          ? "Legacy Moltbook relay reported an active server agent. Phone-native remains the default owner."
          : "Moltbook legacy live bridge reachable."
        : status === "mock"
...
export const publishMoltbookNow = async (
  ...
  if (isMoltbookPhoneNativeMode(state)) {
    const phoneHealth = buildMoltbookPhoneNativeHealth();
    const nativeResult = await callMoltbookPhoneNativePublisher(draft, options.reason || "pocketflow-moltbook-post-button");
    const published = nativeResult.ok && nativeResult.published !== false;
    const message = nativeResult.message || (published
      ? `Posted from phone-native Moltbook runner: ${draft.title}.`
      : "Moltbook draft stayed queued on the phone-native runner.");
    state = {
      ...state,
      apiBaseUrl: MOLTBOOK_PHONE_NATIVE_ENDPOINT,
      connectionMode: "phone_native",
      ...
      postBacklog: published
        ? state.postBacklog.map((itemDraft) =>
        ...
        : state.postBacklog.map((itemDraft) =>
            itemDraft.id === draft.id
              ? { ...itemDraft, status: "planned" as MoltbookDraftStatus, scheduledFor: itemDraft.scheduledFor || now.toISOString() }
              : itemDraft,
          ),
      interaction: {
        ...state.interaction,
        postsToday: state.interaction.postsToday + (published ? 1 : 0),
        totalInteractions: state.interaction.totalInteractions + (published ? 1 : 0),
    ...
    return {
      ...
      draftId: draft.id,
      bridgeItemId: nativeResult.itemId,
      error: published ? undefined : nativeResult.message,
    };
  }
...
export const runMoltbookDueAutomation = async (options: { reason?: string; maxPosts?: number } = {}): Promise<MoltbookAutomationResult> => {
  ...
  if (isMoltbookPhoneNativeMode(state)) {
    ...
    for (const draft of dueDrafts) {
      try {
        const nativeResult = await callMoltbookPhoneNativePublisher(draft, "pocketflow-phone-native-automation");
        if (nativeResult.ok && nativeResult.published !== false) {
          posted += 1;
          postedIds.add(draft.id);
          logEntries.push(`${now.toLocaleString()}: phone-native Moltbook posted "${draft.title}".`);
        } else {
          failed += 1;
          logEntries.push(
            `${now.toLocaleString()}: phone-native Moltbook kept "${draft.title}" queued: ${
              nativeResult.message || "native executor did not confirm publish"
            }`,
          );
        }
      } catch (error) {
        failed += 1;
        logEntries.push(`${now.toLocaleString()}: phone-native Moltbook failed "${draft.title}": ${error instanceof Error ? error.message : String(error)}`);
      }
    }

    const message = posted
      ? `Moltbook phone-native runner posted ${posted} due update${posted === 1 ? "" : "s"}${failed ? `, ${failed} stayed queued` : ""}.`
      : "Moltbook phone-native runner found due posts, but no native publish confirmation was available yet.";
    state = {
      ...state,
      apiBaseUrl: MOLTBOOK_PHONE_NATIVE_ENDPOINT,
      connectionMode: "phone_native",
      connectionHealth: buildMoltbookPhoneNativeHealth(message),
  ...
  if (isMoltbookServerManagedStatus(statusPayload)) {
    state = {
      ...
      interaction: {
        ...state.interaction,
        lastCheckedAt: now.toLocaleString(),
        lastSummary: "Legacy Moltbook relay reported an active server agent. Keep phone-native selected to avoid remote ownership.",
      },
      commandLog: [
        `${now.toLocaleString()}: legacy Moltbook relay reported active server agent; skipped legacy due-post dispatch.`,
        ...state.commandLog,
      ].slice(0, 30),
    };
    saveMoltbookState(state);
    ...
    return {
      ...
      failed: 0,
      skipped: true,
      message: "Legacy Moltbook relay reported an active server agent; phone-native remains the default owner.",
      state,
    };
  }
