...
const getSavedMoltbookEndpoint = (value?: string) => normalizeEndpoint(value || MOLTBOOK_PHONE_NATIVE_ENDPOINT);
...
const buildSavedLiveHealth = (
  ...
  message = "Moltbook route is saved. The phone-native runner owns automation; external bridge is legacy fallback only.",
): MoltbookConnectionHealth => ({
  ...current,
  status: "connected",
  message,
  checkedAt: new Date().toLocaleString(),
  endpoint: endpoint === MOLTBOOK_PHONE_NATIVE_ENDPOINT ? endpoint : `${endpoint}/api/status`,
  serverMode: endpoint === MOLTBOOK_PHONE_NATIVE_ENDPOINT ? "phone-native" : "legacy-live",
  modelProvider: "local",
...
const normalizeVisitorPage = (url: string) => {
  ...
  if (!value) return MOLTBOOK_ACCOUNT_VISITOR_URL;
...
const MoltbookAgentApp = ({ onNotify }: MoltbookAgentAppProps) => {
  ...
  const lastInteractionSummary = state.interaction.lastSummary?.trim()
    || (state.interaction.lastCheckedAt && state.interaction.lastCheckedAt !== "Not synced"
      ...
      : "Not synced yet. Phone-native automation will report after the next native wake.");
  ...
  const addCommandLogEntry = async () => {
    ...
    try {
      ...
      if (instruction.type === "publish_now") {
        setAgentReply("Publishing now. Waiting for phone-native Moltbook confirmation...");
      ...
      setAgentReply(`${result.response} Legacy sync still needs attention: ${message}`);
      onNotify("Instruction saved locally. Legacy sync failed.", "error");
    }
  };
  ...
  const simulateRefresh = () => {
    ...
    onNotify("Moltbook stats refreshed locally from the phone-native state.", "info");
  };
  ...
  const queuePhoneTestDrafts = () => {
    ...
    const phoneTestDrafts: MoltbookPostDraft[] = [1, 2, 3].map((number) => ({
      ...
      body:
        "PocketFlow phone connector test. This draft verifies the mobile Moltbook queue path and must stay held until the phone-native executor confirms live publishing.",
      status: "held",
      source: "manual",
      createdAt,
    }));
    updateState({
      ...
      commandLog: [
        `${now.toLocaleString()}: queued 3 held phone connector test drafts for the phone-native executor.`,
        ...state.commandLog,
      ].slice(0, 80),
    });
    onNotify("Queued 3 phone-native Moltbook test drafts locally.", "info");
  };
  ...
  const runPostTest = async () => {
    ...
    const liveState = {
      ...
      connectionMode: "phone_native" as const,
      connectionHealth: buildSavedLiveHealth(MOLTBOOK_PHONE_NATIVE_ENDPOINT, state.connectionHealth),
    };
    setState(liveState);
    try {
      await syncMoltbookSettingsToBridge(liveState);
    } catch {
      // Phone-native publish below reports the real executor result without falling back to the server relay.
    }
  ...
  const checkConnection = async () => {
    ...
    if (state.connectionMode === "phone_native" || endpoint === MOLTBOOK_PHONE_NATIVE_ENDPOINT) {
      const connectionHealth = buildSavedLiveHealth(
        MOLTBOOK_PHONE_NATIVE_ENDPOINT,
        state.connectionHealth,
        "Phone-native Moltbook runner is armed. External relay is not the automation owner.",
      );
      updateState({ connectionHealth, connectionMode: "phone_native", apiBaseUrl: MOLTBOOK_PHONE_NATIVE_ENDPOINT });
      onNotify("Moltbook phone-native route verified locally.", "success");
      return;
    }
    ...
    try {
      ...
      const connectionHealth = buildSavedLiveHealth(
        endpoint,
        state.connectionHealth,
        `Legacy live route saved. Health probe was slow or unavailable (${detail}); phone-native remains the default owner.`,
      );
      updateState({ connectionHealth, connectionMode: "live", apiBaseUrl: endpoint });
      onNotify("Moltbook legacy route kept for fallback; phone-native remains default.", "info");
    } finally {
      setChecking(false);
    }
  };
        ...
        <section className="overflow-hidden rounded-sm border border-white/10 bg-[#0E0E0E]">
          ...
          <div className="space-y-4 p-5">
            <div className="grid grid-cols-2 gap-3">
              {[
                ["Profile", MOLTBOOK_ACCOUNT_HANDLE],
                ["Followers", "demo"],
                ["Posts", "local"],
                ["Comments", "queued"],
              ].map(([label, value]) => (
            ...
            <div className="rounded-sm border border-emerald-500/20 bg-emerald-500/8 p-4">
              ...
              <p className="mt-3 text-sm leading-6 text-zinc-300">
                This public package ships without a connected live account. Drafts, comment rules, and schedules are local until you wire a private adapter.
              </p>
            </div>
            <button
              onClick={openVisitorPage}
        ...
        <div className="space-y-3">
          ...
          <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            ...
            <div className={`flex min-h-11 items-center justify-center rounded-sm border px-2 py-2 text-center text-[8px] font-mono font-black uppercase tracking-[0.12em] ${healthTone}`}>
              {state.connectionMode === "phone_native" ? "phone native" : state.connectionMode === "live" ? "legacy live" : state.connectionHealth.status}
            </div>
          </div>
        </div>
      </header>
      ...
      <Section title="Connection" subtitle="Account visibility, no secrets printed." defaultOpen={false}>
        <div className="grid grid-cols-1 gap-3">
          <div className="grid grid-cols-3 gap-2">
            <PillButton active={state.connectionMode === "phone_native"} onClick={() => updateState({ connectionMode: "phone_native", apiBaseUrl: MOLTBOOK_PHONE_NATIVE_ENDPOINT })}>
              Phone
            </PillButton>
          ...
          <Field label="Runtime / legacy endpoint" value={state.apiBaseUrl} onChange={(apiBaseUrl) => updateState({ apiBaseUrl })} placeholder="phone-native://moltbook" />
          <Field
